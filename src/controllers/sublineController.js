...
const getAllSubLines = async (req, res) => {
  try {
    ...
    if (retrieveAll) {
      query = `
        SELECT id, lineid, vis, cod, nam, way, main
        FROM "SubLine"
        ${whereClause}
        ${limitClause};
      `;
    ...
    res.status(200).json({
      ...
      sublinesInfo: result.rows, // Send the rows under an explicit 'data' key
    });
  } catch (error) {
...
const getSubLineByCod = async (req, res) => {
  try {
    const { cod } = req.params;
    // Include 'dateNotActive' in the SELECT clause
    const query = 'SELECT id, lineid, vis, cod, nam, way, main FROM "SubLine" WHERE cod = $1';
...
const getSubLineById = async (req, res) => {
  try {
    const { id } = req.params;
    // Include 'dateNotActive' in the SELECT clause
    const query = 'SELECT id, lineid, vis, cod, nam, way, main FROM "SubLine" WHERE id = $1';
